...
# def test_create(start, end, brackets, message, error_type, capfd):
#     if error_type:
...
# def test_eq(first, second, message):
...
#
# @pytest.mark.parametrize(('first', 'second', 'message'), test_cases)
# def test_comparison_lt(first, second, message):
#     r1 = Range(first[0], first[1], first[2])
#     r2 = Range(second[0], second[1], second[2])
#     comparison = r1 < r2
#     assert comparison is message
            ...
            if vi[0] is None and i[0] != '(' or vi[-1] is None and i[-1] != ')':
                ...
                for vj in variations:
                    ...
                    if vj == {}:
                        ...
                        test_cases.append((tuple(vi + [i]), vj, m))
                    elif vj == "":
                        m = "not supported between instances of"
                        if vi == [4., 4.] and i in ("(]", "[)"):
                            m = False
                        ...
                        test_cases.append((tuple(vi + [i]), tuple(vj + [j]), m))


@pytest.mark.parametrize(('first', 'second', 'message'), test_cases)
def test_contains(first, second, message):
    r1 = Range(first[0], first[1], first[2])
    r2 = second
            ...
            if vi[0] is None and i[0] != '(' or vi[-1] is None and i[-1] != ')':
                ...
                for vj in variations:
                    if vj in ({}, None, 5.):
                        ...
                        if is_empty(res):
                            res = Range(empty=True)
                        test_cases.append((tuple(vars(first).values()), tuple(vars(second).values()), res))


@pytest.mark.parametrize(('first', 'second', 'message'), test_cases)
def test_intersect(first, second, message):
...
@pytest.mark.parametrize(('first', 'second', 'message'), test_cases)
def test_add(first, second, message):
    if type(message) == str:
        r1 = Range(first[0], first[1], first[2])
        r2 = second
        if isinstance(second, tuple):
